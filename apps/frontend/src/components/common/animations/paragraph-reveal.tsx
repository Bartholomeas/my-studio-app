...
import React, {type ComponentProps, type PropsWithChildren, useMemo, useRef} from "react";
...
export const ParagraphReveal = ({text, className}: ParagraphRevealProps) => {
    ...
    return (
        <p
            ref={container}
            className={cn("p-md flex flex-wrap items-center text-5xl", className)}
        >
            {words}
        </p>
    );
...
const WordToReveal = ({children, progress, range}: WordToRevealProps) => {
    const opacity = useTransform(progress, range, [0, 1], {ease: cubicBezier(0.17, 0.67, 0.83, 0.67)});
    return (
        <span className={"relative mr-[12px] mt-[12px]"}>
			<span className={"absolute opacity-20"}>{children}</span>
			<motion.span style={{opacity}} className={'duration-500'}>{children}</motion.span>
		</span>
    );
};
