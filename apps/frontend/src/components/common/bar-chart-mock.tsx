...
export const BarChartMock = () => {
	return (
		<div
			className={
				"flex size-full min-h-40 content-end items-end justify-between gap-2 self-end px-4"
			}
		>
			{mockData.map((item, index) => {
				...
				return (
					<div
						key={`barChartMock-${index}`}
						className={"flex h-full flex-col items-center justify-end"}
					>
						<motion.div
							className={clsx("h-full w-[clamp(10px,1rem,3rem)] origin-bottom rounded-full", {
