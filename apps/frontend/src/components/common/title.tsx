...
export const Title = forwardRef<HTMLHeadingElement, TitleProps>(({ type, weight, color, size, align, className, children }, ref) => {
  const Component = type ?? "h2";
  return (
    <Component ref={ref} className={titleVariants({ type: size ?? type, weight, color, align, className })}>
      {children}
    </Component>
  );
});
